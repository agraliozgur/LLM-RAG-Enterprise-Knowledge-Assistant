...
class TinyRAGPipeline:
    ...
    def generate_answer_with_rag(self, user_query: str, retrieved_chunks: list):
        """
    ...
    def generate_answer_llm_only(self, user_query: str):
        """
        Sadece LLM kullanarak yanıt üretir, RAG sürecini atlar.
        """
        prompt = (
            f"Soru: {user_query}\n"
            f"Cevap:"
    ...
    def run_qa_with_rag(self, user_query: str):
        """
        RAG kullanarak Q&A akışı.
        """
        # 1) Query embedding
        ...
        # 3) LLM ile cevap
        answer = self.generate_answer_with_rag(user_query, retrieved)
        return answer

    def run_qa_llm_only(self, user_query: str):
        """
        Sadece LLM kullanarak Q&A akışı.
        """
        answer = self.generate_answer_llm_only(user_query)
        return answer

    def run_both_qa(self, user_query: str):
        """
        Hem RAG hem de LLM-only kullanarak Q&A akışı ve yanıtları döndürür.
        """
        answer_rag = self.run_qa_with_rag(user_query)
        answer_llm = self.run_qa_llm_only(user_query)
        return answer_rag, answer_llm

if __name__ == "__main__":
    # Örnek kullanım:
    pipeline = TinyRAGPipeline(
        embedding_model_name="intfloat/multilingual-e5-small",  # sorgu vektörü
    ...
    while True:
        ...
        if not user_input.strip():
            ...
            break
        # RAG ile cevap
        answer_rag = pipeline.run_qa_with_rag(user_input)
        print(f"\n[With RAG] Yanıt: {answer_rag}\n")
        
        # LLM-only ile cevap
        answer_llm = pipeline.run_qa_llm_only(user_input)
        print(f"[Without RAG] Yanıt: {answer_llm}\n")
